...
### Por qué `corpus.json` se puede publicar

Este repo **se sirve en GitHub Pages**, y la familia de repos ya causó un incidente publicando lo
que no debía. Así que la afirmación «esto es seguro» se sostiene con la lista de campos, no con una
promesa. `corpus.json` contiene, y **nada más**:

```
case_key · resolved_key · case_id · stage_state
update.bumps[]     label · from · to · file · update_kind        ← etiquetas y versiones
update.base_sha · update.head_sha
catalog_origin     corpus_version · repository · base_commit_date
                   requirements · environment_fingerprint
                   dependency_change · probes[] · repair_signal
licence            spdx · resolved_at · url
```

**No hay texto de diff, ni `human_fix`, ni `ground_truth`, ni una línea de código de terceros** —
comprobado sobre el archivo el 2026-08-11 buscando `diff --git`, `+++ b/`, `@@ -`, `human_fix` y
`ground_truth`: cero apariciones. Lo que se publica son metadatos derivados de
`paper_corpus_v1.db`, que es el artefacto **destinado a depositarse**.

Lo que cambia esto en cuanto exista un parche generado: **`warning` deja de poder ser `null`**.
`data.ts` rechaza un bundle con `synthesis` y sin aviso, y hoy no dispara porque no hay ninguno —
que es exactamente cuándo hay que poner un control; después del primer parche sería una corrección.
**Y por eso se prueba rompiéndola**: tres tests la perturban con un `synthesis` sintético y exigen
que rechace sin aviso, que acepte con él, y que hoy no dispare sola. Una guarda que no puede
...
| `update` | paso 2 | **`catalog_contrast`** guarda el contraste contra `dependency_change` **coincida o no**, con tres estados: `null` es «no se comparó» —el caso es ad-hoc—, `agrees: true` es «se comparó y coincide», y `agrees: false` trae el detalle. Sobre los 94 discrepan **4**, y son los 4 `diff-positional`. Un campo que solo apareciera al discrepar colapsaría los dos primeros estados. Más `bumps[]` — cada uno con `label`, `from`, `to`, archivo y `update_kind` (5 valores: `direct`, `plugin-toolchain`, `platform-integration`, `reference-update`, `fallback`) — más `base_sha`/`head_sha` y el diff del bot. **Es una lista incluso cuando trae un solo elemento**, y **10 de los 94** casos traen entre 2 y 4 (3 con dos, 5 con tres, 2 con cuatro). `from`/`to` son **strings opacos**: `"8.1.2"` en un bump de versión, `"f30c8b7"` en uno de referencia — no se ordenan ni se parsean como semver en la vista. Una lista **vacía** significa que el diff no tocó ningún archivo de build reconocible, no "no hubo cambio de versión". Los casos que el corpus cuenta con `bump_count = 0` **son 4 de los 94** —`Oztechan/CCC#2807` y `#4332`, `meshtastic/Meshtastic-Android#5212` y `#5676`, los cuatro `reference-update`—, **pero ésa NO es la cifra de las listas vacías: las listas vacías son cero.** Este documento afirmaba lo contrario hasta el 2026-08-11, cuando `classify_update` existió y se corrió sobre los 94: esos cuatro traen **dos bumps cada uno**, con shas en `from`/`to`. `bump_count` cuenta *versiones*; esta lista cuenta *entradas*. La forma vacía **sí hay que saber dibujarla** —significa «el diff no tocó ningún archivo de build reconocible»— pero es del camino ad-hoc, no del corpus, así que el fixture que la ejercita se escribe a mano y no sale de los 94. El bump primario es nullable y lo llena un paso posterior, nunca la ingesta. Cifras medidas sobre `paper_corpus_v1.db` el 2026-08-06 |
| `execution` | paso 2 | **`probe_diff` ya no es siempre `null`**: desde el paso 3c-3 se compara celda a celda contra los probes del catálogo y **se persiste coincida o no** — `null` es «no se comparó» (caso ad-hoc), `[]` es «se comparó y coincide todo», y una lista con líneas trae las dos versiones de cada celda que discrepa. La vista tiene que distinguir los tres: un campo que solo apareciera al discrepar colapsaría los dos primeros. **El catálogo nunca es fuente**: si discrepan se muestran los dos y se decide fuera. **`build_errors` y `failures` son nullables**: la sección se persiste a medida que salen los probes, así que existe a medias en disco. `null` = «no se llegó a parsear»; `""`/`[]` = «se parseó y no había». La vista tiene que distinguirlas o un caso interrumpido se lee como un build verde. Más: probes por target y revisión **con su nivel** — y los niveles son **cinco: `compile`, `compile-test`, `link`, `test-run`, `ui-exploration`**. **`configuration` NO es uno de ellos**, y esta celda lo listaba como tal hasta el 2026-08-11: es una **compuerta por revisión**, se evalúa antes de enumerar targets, y cuando falla no produce una fila sino que **colapsa esa columna** a `not_reached` celda por celda ([stages.md §2](../../Kmp-Repair-Agents/docs/stages.md)). Dibujarla como nivel haría creer que hay un target llamado «configuration» que se puso rojo. Viaja en `configuration_evaluates`, un booleano por revisión. Más `FailureObservation[]` con rol causal —de los cuatro roles, §2 solo produce **`primary`** y **`preexisting`**; `regression` es de §7, y **`cascade` no lo produce nadie**: es una afirmación causal que un log no sostiene, y asignarlo por orden de aparición era inventarlo—, **texto de error real**, `status` `environment_unavailable` para los targets no ejecutables, hash del log crudo en el `ArtifactStore`, y la comparación contra los probes del catálogo |
...
| `catalog_origin` | **paso 2** (era paso 11) | **Y `null` ya no es hipotético**: desde el paso 3b, `kmp-repair ingest --git owner/name@base..head` produce fichas sin catálogo detrás —`catalog_origin`, `licence` y `catalog_contrast` los tres ausentes— leyendo git en vez de la columna. `data/bundle.adhoc_case.json` es una, y es el mismo caso que uno de los 94, así que la vista se puede comparar lado a lado. **`corpus_sha256`** es el hash del archivo que el pipeline abrió, y la Solicitud §I lo exige como precondición de la Fase 2: *«su SHA-256 identificará la versión exacta utilizada en la experimentación»*. `corpus_version` es la etiqueta que se pidió; el hash dice cuál se leyó — y este corpus se regeneró **cinco veces** sin cambiar de nombre. Una vista que cite «el corpus v1» sin el hash no es trazable. | `null` si el caso no vino del corpus; si vino: `corpus_version`, `case_id`, `ground_truth_files`, `environment_fingerprint`, `licence` y `base_commit_date`. **`ground_truth_files` solo aparece después de congelar la salida** — el dump es post-corrida, así que no rompe A07, pero el orden es parte del control ([evaluation-protocol.md](../../Kmp-Repair-Agents/docs/evaluation-protocol.md)) |
