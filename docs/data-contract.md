...
| `execution` | paso 2 | probes por target y revisión **con su nivel** — y los niveles son **cinco: `compile`, `compile-test`, `link`, `test-run`, `ui-exploration`**. **`configuration` NO es uno de ellos**, y esta celda lo listaba como tal hasta el 2026-08-11: es una **compuerta por revisión**, se evalúa antes de enumerar targets, y cuando falla no produce una fila sino que **colapsa esa columna** a `not_reached` celda por celda ([stages.md §2](../../Kmp-Repair-Agents/docs/stages.md)). Dibujarla como nivel haría creer que hay un target llamado «configuration» que se puso rojo. Viaja en `configuration_evaluates`, un booleano por revisión. Más `FailureObservation[]` con rol causal —`primary`/`cascade`/`preexisting` salen de §2; **`regression` es de §7** y no aparece acá—, **texto de error real**, `status` `environment_unavailable` para los targets no ejecutables, hash del log crudo en el `ArtifactStore`, y la comparación contra los probes del catálogo |
